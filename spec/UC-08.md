...
3. **Execute** — on confirmation, runs in a background thread:
   ...
   - For each `pending` entry: stream-copy source → dest while computing SHA-256 of source in one pass → check hash against DB (existing) and within-batch seen hashes → if duplicate, delete dest and skip → on copy error, delete dest and skip.
