...
| `yyyy-MM-DD-<caption>-####.<ext>` | Date + Caption + Collision Counter |
...
- **`####`**: Exactly 4 digits (e.g., `0001`). Used for sequencing and collision avoidance.
...
```regex
^(\d{4})/(?:\d{4})-(0[1-9]|1[0-2])-(?:(?:0[1-9]|[12]\d|3[01])-(?:[a-z0-9-]+(?:\-\d{4})?|\d{4})|(?:[a-z0-9-]{3,})-\d{4}(?:\-[a-z0-9-]+)?)\.[a-z0-9]+$
```
...
- `(?: ... | ... )`: Two branches for Day-based vs. Slug-based files:
    - **Branch 1 (Day-based)**: Matches `DD-` followed by `caption`, `caption-####`, or just `####`.
